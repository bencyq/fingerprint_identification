...
class ResNet(nn.Module):
    def __init__(self):
        ...
        self.module = nn.Sequential(
            nn.Conv2d(2, 64, kernel_size=7, stride=2, padding=3, bias=False),  # 该卷积层的参数为3*64*7(通道数)，步长为2，填充为3，偏移为0
        ...
        # 添加4层残差
        self.module.add_module('layer1', Residual_block(64, 64, 2, first_block=True))
        self.module.add_module('layer2', Residual_block(64, 128, 2))
        self.module.add_module('layer3', Residual_block(128, 256, 2))
        self.module.add_module('layer4', Residual_block(256, 512, 2))
        # 加入平均池化层
        self.module.add_module('avgpool', nn.AdaptiveAvgPool2d((1, 1)))
        # 加入全连接层
        self.module.add_module('fc', nn.Sequential(
            FlattenLayer(),
            nn.Linear(512, 2)
        ))
        # self.module.add_module('sigmod', nn.Sigmoid())

    def forward(self, input_img):
        x = self.module(input_img)
        return x
